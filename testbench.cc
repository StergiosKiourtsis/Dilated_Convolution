...
int main(){
  ...
  ac_channel<ac_int<8,false> > line_in;
  ac_channel<ac_int<8,false> > line_buff_in;
  ac_channel<ac_int<8,false> > line_out;
  ac_channel<ac_int<8,false> > line_buff_out;
  ...
  dilate<ac_int<8,false>,3,3,7,7>(filter,dil_filter,3,line_in,line_out);         
  for(int i=0;i<7;i++){
	  for(int j=0;j<7;j++){
  ...
  std::cout<<"\n BUFFER TIME! \n";
  convBuf<ac_int<8,false>,10,10,7,7>(array,dil_filter,result2,line_in,line_out,line_buff_in,line_buff_out);
  for(int i=0;i<10;i++){
	  for(int j=0;j<10;j++){
